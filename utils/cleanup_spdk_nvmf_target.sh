# Run cleanup of remote SPDK NVMe-oF target
...
echo "Running remote cleanup on ${REMOTE_USER}@${REMOTE_IP}: sudo ${REMOTE_SCRIPT} cleanup_spdk"

# Use a specific SSH_KEY
SSH_KEY="/home/dedongx/.ssh/id_ed25519"
ssh -i "$SSH_KEY" "${REMOTE_USER}@${REMOTE_IP}" "sudo ${REMOTE_SCRIPT} cleanup_spdk"
