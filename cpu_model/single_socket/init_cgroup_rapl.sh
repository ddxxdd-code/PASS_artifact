...
SOCKET1_CPU_LIST_FILE="/sys/devices/system/node/node0/cpulist"
...
if [ -z "$SOCKET1_CPUS" ]; then
    echo "Error: Could not read non-empty CPU list for socket 0 from ${SOCKET1_CPU_LIST_FILE}." >&2
    exit 1
fi
...
echo "Successfully set cpuset.cpus for ${CGROUP_PATH} to socket 0 CPUs: ${SOCKET1_CPUS}"
