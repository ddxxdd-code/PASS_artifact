...
# WORKLOADS = [
#     "fillseq", "fillsync", "overwrite"
# ]

def parse_log_file(file_path):
    """
    Return a dict:
        { workload: (throughput_ops, p99_latency_us) }
    ...
    for wk in WORKLOADS:
        # 1) throughput (ops/sec) — comes on the same line as micros/op
        m_thr = re.search(rf"{wk}\s*:.*?(\d+)\s+ops/sec", text, re.I)
        thr   = int(m_thr.group(1)) if m_thr else None

        # 2) 99-th percentile latency — appears later in the same section
        m_p99 = re.search(
            rf"{wk}.*?Percentiles:.*?P99:\s*([0-9.]+)",
            text, re.I | re.S)
...
def process_logs(method, powercap):
    ...
    # newest timestamp among files like 1684284962_db_bench_3.log
    ts_set = {
        m.group(1)
        for m in (re.match(r"(\d+)_db_bench_\d+.log", f) for f in os.listdir(directory))
        if m
    }
    if not ts_set:
        print(f"No log files in '{directory}'.")
        return
    latest_ts = max(ts_set)

    # Parse every disk log
    per_disk = []
    ...
    if not per_disk:
        print(f"No valid logs for timestamp '{latest_ts}'.")
        return
    ...
    # average p99 over disks
    avg_p99 = {
        wk: (mean(vals) if vals else None)
        for wk, vals in agg_p99.items()
    }

    # ── write CSV ──
    out_csv = os.path.join(directory,
                           f"{latest_ts}_aggregated_metrics.csv")
    with open(out_csv, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow([
            "method", "powercap", "workload",
            "throughput (ops/s)", "p99 latency (µs)"
        ])
        for wk in WORKLOADS:
            w.writerow([
                method, powercap, wk,
...
def main():
    ap = argparse.ArgumentParser(
        description="Aggregate db_bench throughput & p99 latency.")
    ap.add_argument("-s", "--scheduler", required=True,
                    help="Scheduler/method name.")
    ap.add_argument("-c", "--powercap",  required=True,
                    help="Power cap value (e.g., 360, unlimited).")
    args = ap.parse_args()
